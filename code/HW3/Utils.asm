...
;void Open(int sector,int head,int count,int address);
	...
	mov ah, 02h     										;功能号
	mov cl, byte[bp + (BIAS_CALL+BIAS_PUSH*5+BIAS_ARG*0)]   ;起始扇区号
	mov ch, 0												;柱面号
	mov al, byte[bp + (BIAS_CALL+BIAS_PUSH*5+BIAS_ARG*2)]   ;扇区数 
	mov dl, 0             									;驱动器号
	mov dh, byte[bp + (BIAS_CALL+BIAS_PUSH*5+BIAS_ARG*1)]  	;磁头号
